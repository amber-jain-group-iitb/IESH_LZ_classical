...
subroutine hop_LZ
  ...
  if(j==0) then
    do i=1,n_el
      if((V_k(state(i))-V_k(1))*(V_k_old(state(i))-V_k_old(1))<0.d0) then
        p_LZ=abs(2*pi*Hamil_diab(state(i),1)**2/(hbar*v(1)*2*g_coup1))
        call random_number(rnd)
!        if(rnd<p_LZ) write(6,*) "unocc",curr_time*1.d15,state(i),1,(V_k(state(i))-V_k(1))/wave_to_J
    ...
    do i=1,nquant
      if(.not.(any(i==state))) then
        if((V_k(i)-V_k(1))*(V_k_old(i)-V_k_old(1))<0.d0) then
          p_LZ=abs(2*pi*Hamil_diab(i,1)**2/(hbar*v(1)*2*g_coup1))
          call random_number(rnd)
...
subroutine evolve_quantum_small_dtq
  ...
  integer i,nstep_qm
  real*8 dtq1,dtq2
  real*8 V_k_hold(nquant),dVk_dt(nquant),dh_dt(nbasis,nbasis)
  real*8 dforce_dt(nquant,nclass)
  complex*16 ci_prev(nquant),dci_dt(nquant)

  !call compute_vdotd
  !dVk_dt=(V_k-V_k_old)/dtc
  dH_dt=(Hamil_diab-Hamil_diab_old)/dtc
!  if(icollapse==1) then
  ...
  do j=1,n_el
    do i=1,nquant
      if(.not.(any(i==state))) then
        state_tent=state
        state_tent(j)=i
        call calculate_Akj(AA,state_tent,state)
        !pr=-2*real(conjg(AA))*vdotd(i,state(j))
        pr=2*dimag(conjg(AA))*Hamil_diab(i,state(j))/hbar
...
subroutine rk4(ci,dtq,dH_dt)
  implicit none
  complex*16,intent(inout)::ci(nquant,n_el)
  real*8,intent(in) :: dtq,dh_dt(nbasis,nbasis)!,dVk_dt(nquant)
  ...
  Hamil_diab=Hamil_diab+dH_dt*dtq/2.d0
  call compute_mat_diab

  k2=matmul(mat,ci+0.5*dtq*k1)
  k3=matmul(mat,ci+0.5*dtq*k2)

  !V_k=V_k+dVk_dt*dtq/2.d0
  Hamil_diab=Hamil_diab+dH_dt*dtq/2.d0
  call compute_mat_diab

  k4=matmul(mat,ci+dtq*k3)
...
subroutine evolve_classical(dt)
  ...
  if(ifriction==1) then
    ...
    c2=1.d0/gama_dt*(1.d0-c1)
     ...
     x(1)=x(1)+dt*v(1)+0.5*dt*dt*acc(1)
     x(2)=x(2)+c1*dt*v(2)+c2*dt*dt*acc(2)+delta_r(2)
     acc_old=acc
     call evaluate_variables(0)
     v(1)=v(1)+0.5*dt*(acc(1)+acc_old(1))
...
subroutine write_average
  ...
  do i=1,nsteps/nstep_avg
   ...
   ! enddo
   write(100,'(21f17.2)')(i-1)*nstep_avg*dtc*1.d12,pop(1,i)!rho(1,1,i)
  enddo
...
subroutine tise
  ...
  do j=1,n_el
    do i=1,nclass
      ...
      acc_qm(i)=acc_qm(i)-delH_dels(state(j),state(j),i)/mass(i)
    enddo
...
subroutine potential_classical(pot_cl,acc_cl)
  ...
  pot_cl=0.5*mass(1)*omg1**2*(x(1)+g_coup1/(mass(1)*omg1**2))**2
  !pot_cl=pot_cl + 0.5*mass(1)*omg2**2*(x(2)-g_coup2*x(1)/(mass(1)*omg2**2))**2
  pot_cl=pot_cl + 0.5*mass(1)*omg2**2*x(2)**2-g_coup2*x(1)*x(2) + g_coup2**2/(2*mass(1)*omg2**2)*x(1)**2
  acc_cl(1)=mass(1)*omg1**2*(x(1)+g_coup1/(mass(1)*omg1**2)) +  g_coup2**2/(mass(1)*omg2**2) *x(1)   - g_coup2*x(2)
  acc_cl(2)=mass(1)*omg2**2*x(2)  - g_coup2*x(1)
...
!  state(n_el)=state(n_el)+2
  ...
  do i=1,1000
    x(1)=-1d-10+2.d-10*i/999.d0
    x(2)=g_coup2*x(1)/(mass(1)*omg2**2)
    call evaluate_variables(0)
    write(20,*) x(1)*1.d10,V_k(1:6)/wave_to_J
